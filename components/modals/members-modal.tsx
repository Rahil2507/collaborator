...
export const MembersModal = () => {
  ...
  const [loadindId, setLoadindId] = useState("")
    ...
    <Dialog open={isModalOpen} onOpenChange={onClose}>
      <DialogContent className="bg-white text-black overflow-hidden">
        <DialogHeader className="pt-8 px-6">
          ...
          <DialogDescription className="text-center to-zinc-500"> {server?.members?.length} Members </DialogDescription>
        </DialogHeader>
        <ScrollArea className="mt-8 max-h--[420px] pr-6">
          {server?.members?.map(member => (
            <div key={member.id} className="flex items-center gap-x-2 mb-6">
              ...
              {server.profileId !== member.profileId && loadindId !== member.id && (
              ...
              {loadindId === member.id && ( <Loader2 className="animate-spin text-zinc-500 ml-auto w-4 h-4" /> )}
            </div>
          ))}
        </ScrollArea>
      </DialogContent>
