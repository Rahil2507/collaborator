...
export const ChatItem = ({id, content, member, timestamp, fileUrl, deleted, currentMember, isUpdated, socketUrl, socketQuery}: ChatItemProps) => {
  ...
  const isCode = fileType === "code" && fileUrl
  const isImage = !isPDF && !isCode && fileUrl
      ...
      <div className="group flex gap-x-2 items-start w-full">
        ...
        <div className="flex flex-col w-full">
          <div className="flex items-center gap-x-2">
            ...
            <span onClick={onMemberClick} className="text-xs text-zinc-500 dark:text-zinc-400">{timestamp}</span>
          </div>
          {isCode && (
            <div className='w-full md:w-96 bg-black m-5 p-5 pt-10 rounded-lg'>
                  ...
                  <FormItem className="flex-1">
                    <FormControl>
                      <div className="relative w-full">
                        <Input disabled={isLoading} {...field} placeholder="Edited message" className="p-2 bg-zinc-200/90 dark:bg-zinc-700/75 border-none border-0 focus-visible:ring-0 focus-visible:ring-offset-0 text-zinc-600 dark:to-zinc-200" />
                      </div>
                    </FormControl>
                  </FormItem>
                )} />
      ...
      {canDeleteMessage && (
        <div className="hidden group-hover:flex items-center gap-x-2 absolute p-1 -top-2 right-5 bg-white dark:bg-zinc-800 border rounded-sm">
          ...
          <ActionTooltip label="Delete">
            <Trash onClick={() => onOpen("deleteMessage", {apiUrl: `${socketUrl}/${id}`, query: socketQuery})} className="cursor-pointer ml-auto h-4 w-4 text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 transition"/>
          </ActionTooltip>
        </div>
      )}
    </div>
